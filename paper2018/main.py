...
class Noise_model(nn.Module):
    ...
    def forward(self, x, debug= False, no_noise= False):
        ...
        new_input = apply_delta / 32767 + original
        if no_noise:
...
def DB(x):
	return 20 * torch.max(x).log().clamp(min=-5.0,max=1000) / torch.log(torch.tensor(10.0))
...
for i in range(len(audios)):
	...
	optimizer_noise = torch.optim.Adam(noise_model.parameters(), lr=0.001, weight_decay=1)
	...
	for k in range(100) :
		...
		for j in range(args.num_iterations):
			...
			db_x = torch.abs(db_x)
			ctc_loss = criterion(softmax(float_out).to(torch.float64).cpu(), targets, output_sizes, target_sizes).to(device)
			if db_x.item() < 10**(tau/20) :  
				loss = 10*ctc_loss
			else:
				loss = 10* l2_norm +  100 * (db_x - 10**(tau/20)) +   ctc_loss
			model.zero_grad()
			...
			if j % 100 == 0 : 
				noise_model.rescale = noise_model.rescale/1.05
				...
				if loss_value < prev and loss_value > prev -0.25:
					count += 1
					prev = loss_value
					if count >2:
						break	 
